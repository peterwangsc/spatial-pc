...
A subsequent actual LAN probe exercised reviewed host revision `18eaf71` and the same native helper. The paired Mac Python sender negotiated text v1 and sent three fixed scalars (Latin, CJK and supplementary) into a dedicated Windows observer. The observer confirmed four expected UTF-16 `WM_CHAR` units, four Unicode key downs, four ups and zero unexpected events. The session sent six control records, received 77 real desktop video frames including 28 after stop, and ended with native exit 0 and no remaining capture/helper children. No Return, physical-key or pointer actions were sent; no desktop images or input contents were logged. A first attempt sent nothing because an SSH process could not inspect the interactive desktop foreground. The corrected test guard verified the observer process and its fresh, atomically published foreground checks from the interactive session. This is a Mac transport-to-Windows integration test; it does not establish physical visionOS keyboard delivery.
