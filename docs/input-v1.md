...
For an explicitly reviewed lab session, add `--enable-input --input-bridge PATH_TO_INPUT_BRIDGE` to the existing host command. Keep the tested capture binary and pairing unchanged. This implementation has component and native-window validation; paired visionOS input and a live opt-in host switch remain separate coordinated checks.
