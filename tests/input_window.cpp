...
struct Observed {int down=0,up=0,buttonsDown=0,buttonsUp=0,wheel=0,horizontal=0;POINT move{-999,-999};std::vector<WORD> chars;} observed;
LRESULT CALLBACK windowProc(HWND window,UINT message,WPARAM w,LPARAM l){
    ...
    case WM_CHAR:if(observed.chars.size()<16)observed.chars.push_back(WORD(w));return 0;
    case WM_SYSCHAR:case WM_CONTEXTMENU:return 0;
...
int wmain(int argc,wchar_t** argv){
    ...
    try{
        const bool textTest=argc==3&&std::wstring(argv[2])==L"--text";
        const bool leaseTest=argc==3&&std::wstring(argv[2])==L"--lease";
        require(argc==2||textTest||leaseTest);
        ...
        if(textTest)command+=L" --enable-text";
        ...
        if(textTest){
            event(5,0);event(8,0,0xe9);event(8,0,0x4e2d);event(8,0,0x1f642);
            stage="Unicode WM_CHAR receipt";require(GetForegroundWindow()==window);DWORD written=0;
            require(WriteFile(inputWrite,bytes.data(),DWORD(bytes.size()),&written,nullptr)&&written==bytes.size());
            const auto injected=GetTickCount64();while(GetTickCount64()-injected<1500&&observed.chars.size()<4){pump();require(GetForegroundWindow()==window);Sleep(5);}
            require(observed.chars==std::vector<WORD>({0xe9,0x4e2d,0xd83d,0xde42}));
            CloseHandle(inputWrite);inputWrite=nullptr;
            const auto closed=GetTickCount64();while(GetTickCount64()-closed<3000&&WaitForSingleObject(child.hProcess,0)!=WAIT_OBJECT_0){pump();require(GetForegroundWindow()==window);Sleep(5);}
            pump();DWORD code=99;require(GetExitCodeProcess(child.hProcess,&code)&&code==0);
            require(observed.down==4&&observed.up==4&&observed.buttonsDown==0);
            std::cout<<"PASS: real Unicode helper delivers BMP and supplementary UTF16 WM_CHAR sequence with balanced key release in dedicated window\n";exitCode=0;
        }else{
