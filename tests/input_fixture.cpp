...
#include "../windows/host/CapturePipeOwner.h"
void u32(unsigned char* out,uint32_t value){for(int i=0;i<4;++i)out[i]=BYTE(value>>(24-8*i));}
int main(int argc,char** argv){
    bool continuous=false,textEnabled=false;
    for(int i=1;i<argc;++i){continuous|=std::string(argv[i])=="--until-owner-exits";textEnabled|=std::string(argv[i])=="--enable-text";}
    if(argc>=2&&std::string(argv[1])=="--stream"){
        std::unique_ptr<CapturePipeOwner> owner;
        try{owner=std::make_unique<CapturePipeOwner>(continuous);}catch(const std::exception&){return 2;}
        ...
        const bool idle=GetEnvironmentVariableA("SPATIAL_INPUT_FIXTURE_IDLE",nullptr,0)>0;
        std::vector<unsigned char> payload(flood?1048576:8,0);payload[3]=1;payload[4]=0x65;
        for(uint64_t frame=0;owner->alive()&&(continuous||frame<3600);++frame){
            if(idle){Sleep(100);continue;}
    ...
    try{
        ...
        while(continuous||GetTickCount64()-start<10000){
